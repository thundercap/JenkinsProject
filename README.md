...
--------------------------------------------
Repo URL: https://github.com/thundercap/JenkinsProject.git
...
pipeline {
    ...
    stages {
        ...
        stage ("Git Checkout") {
            steps {
                git 'https://github.com/thundercap/JenkinsProject.git'
            }
        }
            ...
            steps {
                script {
                    withDockerRegistry(credentialsId: 'docker') {
                        sh "docker tag zomato spygod/zomato:latest "
                        sh "docker push spygod/zomato:latest "
                    }
                }
            }
        }
            ...
            steps {
                script{
                   withDockerRegistry(credentialsId: 'docker', toolName: 'docker'){
                       sh 'docker-scout quickview spygod/zomato:latest'
                       sh 'docker-scout cves spygod/zomato:latest'
                       sh 'docker-scout recommendations spygod/zomato:latest'
                   }
                }
            }
        }
        stage ("Deploy to Container") {
            steps {
                sh 'docker run -d --name zomato -p 3000:3000 spygod/zomato:latest'
            }
        }
    }
    post {
...
# Create Cluster. I will keep the cluster name as "zomatocluster"
eksctl create cluster --name=zomatocluster \
...
eksctl create cluster --name=zomatocluster --region=ap-northeast-1 --zones=ap-northeast-1a,ap-northeast-1c --without-nodegroup
...
To verify the cluster creation ---> Goto Cloud Formation service in AWS ----> You should see a stack got created with the name "zomatocluster". Make sure in the vs code editor the cluster will get created. As said earlier it will take atleast 20 minutes.
Once the cluster is ready, you will see "EKS Cluster "zomatocluster" in "us-east-1" region is ready" in vs code editor. wait till you see this.

...
eksctl utils associate-iam-oidc-provider \
    --region ap-northeast-1 \
    --cluster zomatocluster \
    --approve

(OR)
eksctl utils associate-iam-oidc-provider --region ap-northeast-1 --cluster zomatocluster --approve
...
eksctl create nodegroup --cluster=zomatocluster \
                       --region=ap-northeast-1 \
                       --name=zomatodemo-ng-public1 \
                       --node-type=t3.medium \
                       --nodes=2 \
                       --nodes-min=2 \
                       --nodes-max=4 \
...
(OR)
eksctl create nodegroup --cluster=zomatocluster --region=ap-northeast-1 --name=zomatodemo-ng-public1 --node-type=t3.medium --nodes=2 --nodes-min=2 --nodes-max=4 --node-volume-size=20 --ssh-access --ssh-public-key=Prajwal --managed --asg-access --external-dns-access --full-ecr-access --appmesh-access --alb-ingress-access
...
eksctl get nodegroup --cluster=zomatocluster

# Delete Node Group
eksctl delete nodegroup --cluster=<clusterName> --name=<nodegroupName>
eksctl delete nodegroup --cluster=zomatocluster --name=zomatodemo-ng-public1

Step 07: Delete Cluster
# Delete Cluster
eksctl delete cluster <clusterName>
eksctl delete cluster zomatocluster
********************************************************************************
********************************************************************************
